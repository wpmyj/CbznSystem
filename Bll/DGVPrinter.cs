...
namespace Bll //AllocationRequest
{
    ...
    public static class Extensions
    {
        ...
        public static void DrawImbeddedImage<T>(List<T> list,
    ...
    public class DGVPrinter
    {
        ...
        private bool PrintPage(Graphics g)
        {
            ...
            List<ImbeddedImage> listimbeddedimage;
            ...
            if (PrintHeader)
            {
                ...
                listimbeddedimage = new List<ImbeddedImage>();
                foreach (var item in ImbeddedImageList)
                {
                    if (item.ImageLocation == Location.Header)
                    {
                        listimbeddedimage.Add(item);
                    }
                }
                Extensions.DrawImbeddedImage(listimbeddedimage, g, printWidth, pageHeight, pagesets[currentpageset].margins);
            ...
            foreach (var item in ImbeddedImageList)
            {
                if (item.ImageLocation == Location.Absolute)
                {
                    listimbeddedimage.Add(item);
                }
            }
            ...
            while ((printpos + nextrowheight) < staticheight)
            {
                ...
                if (lastrowprinted >= rowstoprint.Count - 1)
                {
                    ...
                    foreach (var item in ImbeddedImageList)
                    {
                        if (item.ImageLocation == Location.Footer)
                        {
                            listimbeddedimage.Add(item);
                        }
                    }
                ...
                if (cellwidth > 0)
                {
                    ...
                    if (!DrawOwnerDrawCell(g, row.Cells[col.Index], cellprintarea, colstyle))
                    {
                        ...
                        cellprintarea = new RectangleF(xcoord + colstyle.Padding.Left,
                            ...
                            rowheights[lastrowprinted] - colstyle.Padding.Bottom - colstyle.Padding.Top);
